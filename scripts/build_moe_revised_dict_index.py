...
PINYIN_PATTERNS = ("拼音",)
TEXT_PATTERNS = (
    "釋義", "释义", "內容", "内容", "解釋", "解释", "義項", "义项", "辨似", "辨識", "附注", "附錄", "附录",
    "書證", "书证", "例句", "出處", "出处", "補充", "补充",
)
AUXILIARY_TEXT_PATTERNS = (
    "相似詞", "相似词", "相反詞", "相反词", "似", "反", "異體", "异体", "部首", "總筆畫", "总笔画", "部外筆畫", "部外笔画",
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a local read-only SQLite index for 教育部《重編國語辭典修訂本》."
    )
...
def write_database(output_path: Path, normalized_rows: list[dict], metadata: dict) -> None:
    ...
    try:
        ...
        con.execute("CREATE INDEX idx_moe_entries_headword_norm ON entries(headword_norm)")
        con.execute("CREATE INDEX idx_moe_entries_headword ON entries(headword)")
        con.execute("CREATE INDEX idx_moe_entries_hash ON entries(content_hash)")
        con.executemany(
            """
            INSERT INTO entries (
